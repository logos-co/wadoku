...
//	hostAddr, _ := net.ResolveTCPAddr("tcp", tcpEndPoint)
	...
	subNode, err := pubsub.NewGossipSub(ctx, host)
	if err != nil {
		panic(err)
	}
	...
	topic, err := subNode.Join(conf.ContentTopic)
	if err != nil {
		panic(err)
	}

  subTopic, err := topic.Subscribe()
	if err != nil {
		panic(err)
	}

	// create publisher
	go subscribe(subTopic, ctx, host.ID())
...
func subscribe(subTopic *pubsub.Subscription, ctx context.Context, localHost peer.ID) {
	for {
		msg, err := subTopic.Next(ctx)
		if err != nil {
			panic(err)
		}

		// only consider messages delivered by other peers
		if msg.ReceivedFrom == localHost {
			continue
		}

		log.Info("received ", string(msg.Data), " from ", msg.ReceivedFrom.Pretty())
	}
}
